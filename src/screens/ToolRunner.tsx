...
export default function ToolRunner({ toolId, children }: { toolId: string; children?: ReactNode }) {
  const posthog = usePostHog();
  const tool = toolId ? TOOL_BY_ID.get(toolId) : undefined;
  const op = tool?.op ? OPS[tool.op] : undefined;
  const pageContent = tool ? getToolContent(tool.id) : undefined;
  ...
  // Track tool opens; route metadata is generated by the Next.js server page.
  useEffect(() => {
    if (tool) {
    ...
    <div className="page page--tool">
      ...
      >
        ...
        <div className="dropzone__inner">
          ...
          {!hasFiles && <p className="dropzone__hint">{isPdfInput ? 'This PDF is processed in your browser and is not uploaded to opentools.' : isCombine ? 'All images combine into one output.' : 'Editor & live preview appear below once you add an image.'}</p>}
        </div>
      </div>
